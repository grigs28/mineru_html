...
### 🐛 修复"开始转换"点了没反应
- **根因 1（后端）**：`clear_all` 直接置队列 IDLE 但没重置工人标志，工人退出后 `_workers_started` 残留 True → 后续启动队列不拉起工人 → 任务永远 QUEUED 空转；改为走 `stop_queue()`
- **根因 2（前端）**：`startConversion`/`updateConvertButtonState` 把"队列运行中"当异常拦截（串行时代遗留），而队列常驻运行是正常设计——按钮不再禁用，上传入队即由工人消化
- 新增回归测试：stop_queue 必须重置工人标志

## [0.9.5] - 2026-09-09

### 🐛 修复转换完成后文件大小显示 0 Bytes
...
### 🐛 修复来源徽章不显示
- 前端三条数据通路补齐 `origin` 字段：本地创建（UI 上传即标 ui）、服务端列表合并、任务状态轮询（updateFileDataFromTask）
- 轮询局部刷新路径补插徽章（此前只有整卡重建才渲染）
- 徽章插入改用安全 DOM API（createElement/textContent）

## [0.9.3] - 2026-09-09

### 🐛 修复登录回跳
- yz-login 按注册应用 URL 跳回**根路径** `/?ticket=xxx`，v0.9.1 只在 `/callback` 验票导致未登录循环；现 `GET /` 带 ticket 参数时直接走验票签发流程
...
### ✨ 队列徽章语义补全
- 新增「待启动」状态：有任务排队但队列未启动时显示 🟠 待启动 · n 排队（此前显示"队列空闲"，易造成"有任务却没在跑"的误解）
- `/file_parse` 同步转换不经队列的盲区：有 processing 状态文件时徽章也显示"转换中"

## [0.8.1] - 2026-09-04

### 🐛 修复
- **外部调用方 URL 编码文件名导致 500**：上传接口（`/api/upload_with_progress`、`/file_parse`、`/convert_to_pdf`）收到 `%XX` 编码形态文件名时先 `unquote` 还原再落盘，根治"编码后每汉字膨胀 9 字节、超长中文名突破 255 字节上限报 Errno 36"导致的上传失败
- 文件名显示截断长度收紧（卡片 38 字符、输出目录 50 字符），修复轮询局部刷新时错误信息块被进度条覆盖的问题
- **外部 API 上传文件大小显示 0 Bytes**：`/api/file_list` 对 size 为 0 的条目从磁盘上传文件 stat 真实大小回填（含历史条目），不再写死 0

## [0.8.0] - 2026-09-04

### 🐛 前端显示修复
- **文件名 URL 编码显示**：部分外部调用方上传时文件名已是百分号编码形态（如 `GB%2050736-2012%20...pdf`），卡片/详情/下载提示/输出列表统一解码后显示（原始名仍作数据键与 API 参数，后端存储逻辑不变）
- **文件名长度控制**：中间截断保留首尾与扩展名（`GB 50736-2012 民用建筑…设计规范(书签版).pdf`），CSS 省略号作双保险
- **失败原因可见**：任务卡片直接展示后端已下发的 `errorMessage`（此前只显示"❌ 失败/错误"，无法获知失败原因）
- **工具栏按钮溢出修复**：文件列表 / 输出目录的头部按钮组（刷新/全选/删除/打包下载等）在窄面板下换行显示，不再被挤出屏幕
- **XSS 加固**：文件名、错误信息等用户可控文本插入 innerHTML 前统一 HTML 转义（`MinerUUtils.escapeHtml`）

### ✨ 新功能
- **队列运行状态徽章**：header 新增实时状态指示（🟢 转换中 · n 排队 / ⚪ 队列空闲 / 🟡 已暂停），复用 `/api/queue/status` 轮询，一眼可见是否有转换任务运行

### 🎨 UI 美化
- header 重设计：左侧品牌区（M 标识 + 标题/副标题），右侧队列徽章 + 版本号
- 控制面板分组卡片化：「基础设置」（页数/后端引擎+说明）与「识别选项」（公式/表格）
- 按钮体系统一：主操作渐变实心，次要操作描边，危险操作红色描边，36px 等高
- 表单控件 hover/focus 态（聚焦光圈）、上传区/卡片视觉统一（白底 + 细边框 + 轻阴影）

### ⬆️ 依赖升级
- **MinerU 3.3.x → 3.4.5**：PDF 文本提取代理对 Unicode 修复、字体分析增强、DOCX 表格/样式修复等（无破坏性变更，vllm 0.21.0 兼容 `<0.22.0` 约束）
- 基础镜像 `mineru-base:3.3` → `mineru-base:3.4`，pip 锁区间 `mineru[core]>=3.4.5,<3.5`
